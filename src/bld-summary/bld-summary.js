...
//    await processCubingChinaResults(stats, today, oneWeekAgo, resultsToBeat);
  ...
  try {
    const embeds = [...makeSummaryEmbeds(stats), makeStatsEmbed(stats)];
    const recordsChannel = await client.channels.fetch(
      process.env.recordsChannelId
    );
    await recordsChannel.send({ embeds });
  } catch (error) {
...
function makeSummaryEmbeds(stats) {
  ...
  const embeds = lines.reduce((acc, cur) => {
    const lastLen = acc.at(-1)?.totalLength || null;
    if(lastLen === null || lastLen+cur.length+1 > 4096){ acc.push({lines: [], totalLength: 0}); }
    const last = acc.at(-1);
    if(last){ 
      last.lines.push(cur); 
      last.totalLength += cur.length + 1; // +1 for \n later
    }
    return acc;
  }, []).flatMap((item, i) => {
    if(item.totalLength === 0){return [];} // idk why it would be 0
    const eb = new EmbedBuilder();
    eb.setColor(0x7289dd);
    if(i===0){ eb.setTitle("Weekly BLD Summary"); }
    eb.setDescription(item.lines.length > 0 ? item.lines.join("\n") : "No results");
    return [eb];
  });
